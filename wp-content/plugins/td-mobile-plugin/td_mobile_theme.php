...
class td_mobile_theme {
	...
	static function set_the_theme() {
		...
		if ( self::get_option('tdm_amp') !== '' ) {
			...
			if ( self::is_amp_plugin_installed() ) {
                ...
                // we could not find another option to stop loading this in wp-admin, so we check the server request
				if ( self::get_option('tdm_amp') === 'mobile_amp' && strpos($_SERVER['REQUEST_URI'], '/wp-json/wp/v2/') === false ) {
	...
	static function mobile_theme_setup() {
        ...
        /**
         * WP 6.4.0 fix for - Memorizes filter execution so that it only runs once for the current theme
         * Reset globals $wp_template_path and $wp_stylesheet_path to force refresh the path

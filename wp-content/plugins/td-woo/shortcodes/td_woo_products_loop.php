...
class td_woo_products_loop extends td_block {
    ...
    static function cssMedia( $res_ctx ) {
        ...
        $modulo_posts = (int)self::$products_count % intval((100/intval($modules_number)));
...
//        }
    ...
    function render( $atts, $content = null ) {
        ...
        if ( !empty($atts['use_woo_pagination']) ) {
            $atts['limit'] = wc_get_default_products_per_row() * wc_get_default_product_rows_per_page();
            $atts['ajax_pagination'] = 'numbered';
            $atts['offset'] = 0;
            $atts['products_ids'] = '';
        }
        ...
        self::$products_count = !empty( $this->td_query['ids'] ) ? count( $this->td_query['ids'] ) : 0;
        ...
        $buffy .= '<div class="' . $this->get_block_classes( $additional_classes_array ) . '" ' . $this->get_block_html_atts() . '>';
	        ...
	        if ( ! ( tdc_state::is_live_editor_iframe() || tdc_state::is_live_editor_ajax() ) ) {
		        ...
		        // render js
                td_resources_load::render_script( TD_WOO_SCRIPTS_URL . '/tdwLoop.js' . TD_WOO_SCRIPTS_VER, 'tdwLoop-js', '', 'footer' );
		        ob_start();
            ...
            if ( $pagination != '' ) {
                if ( $pagination === 'numbered' ) {
	                if ( tdb_state_template::get_template_type() === 'woo_shop_base' ||
                        ...
                        $loop_data['loop_pagination']['is_woo_pagination'] = $td_woo_loop_products_data['is_woo_pagination'];
    ...
    function get_numbered_pagination( $loop_pagination_data ) {
        ...
        //when use_woo_pagination is active /page/# will be used instead of ?product-page=#
        if ( $pagination_data['is_woo_pagination'] ) {
            $base = ( td_woo_util::is_ajax() ) ? esc_url_raw( add_query_arg( 'product-page', '%#%', !empty($pagination_data['base']) ? $pagination_data['base'] : '' ) ) : esc_url_raw( str_replace( 999999999, '%#%', remove_query_arg( 'add-to-cart', get_pagenum_link( 999999999, false ) ) ) );
        }

        $format  = $pagination_data['is_woo_pagination'] ? '' : '?product-page=%#%';

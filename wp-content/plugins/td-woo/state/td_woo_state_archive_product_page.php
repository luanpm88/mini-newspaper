 ...
 * @property tdb_method archive_product_custom_field
 *
 *
 */
...
//            var_dump($menu_id);
		...
		$this->loop = function ($atts) {
            ...
            } elseif ( !empty($atts['use_woo_pagination']) ) {
			...
			if ( !empty($paged) && $paged > 1 && isset( $limit ) ) {
            ...
            $current_page = !$is_woo_pagination ? intval( $td_woo_loop_products_data['current_page'] ) : $paged;
			...
			$td_woo_loop_products_data['loop_pagination']['is_woo_pagination'] = $is_woo_pagination;
        ...
        // custom field
        $this->archive_product_custom_field = function($atts) {
            ...
            $archive_object = $this->woo_archive_queried_obj;
            ...
            if( $field_name != '' ) {
                $field_data = td_util::get_acf_field_data($field_name, $archive_object);

                //print_r($field_name);
                //var_dump($field_data);

                if( !$field_data['meta_exists'] ) {
                    if( metadata_exists('term', $archive_id, $field_name) ) {
                        $field_data['value'] = get_term_meta($archive_id, $field_name, true);

...
class tds_util {
						...
						case 'trial':
							if ( !empty($subscription['trial_days']) ) {
								...
								if ( $strtotime_now > strtotime('+' . $subscription['trial_days'] . ' days', date_timestamp_get($start_date) ) ) {
                                    ...
                                    // maybe apply coupon on subscription price
                                    $price = !empty($subscription['coupon_id']) ? tds_util::get_coupon_discount( $subscription['coupon_id'], $subscription['price'] ) : intval( $subscription['price'] );
                                    $status = $price === 0 ? 'active' : 'waiting_payment';
							...
							if ( $strtotime_now > date_timestamp_get( $start_date_next_next_cycle ) ) {
								...
								if ( false !== $update_result && !$canceled ) {
									...
									if ( $data_values['payment_type'] === 'stripe' ) {
                                        ...
                                        // keep stripe subscription price, as a price change on stripe subscriptions is not supported
                                        $data_values['price'] = $subscription['price'];
								...
								if ( false !== $update_result && !$canceled ) {
									...
									if ( $data_values['payment_type'] === 'stripe' ) {
                                        ...
                                        // keep stripe subscription price
                                        $data_values['price'] = $subscription['price'];
									...
									if ( false !== $insert_result ) {
                                        ...
                                        // run custom payments on subscription renew hooks
                                        do_action( 'tds_subscription_' . $data_values['payment_type'] . '_renew', $wpdb->insert_id );
                    ...
                    if( !$subscription['is_free'] ) {
                        switch( $subscription['payment_type'] ) {
                            ...
                            default:
                                ...
                                $subscription['formatted_payment_type'] = apply_filters( 'tds_subscription_' . $subscription['payment_type'] . '_name', $subscription['payment_type'] );

                                // add subscription custom payment_type additional details
                                $subscription['custom_payment_details'] = apply_filters( 'tds_subscription_' . $subscription['payment_type'] . '_custom_details', '', $subscription );
	...
	static function get_country_iso_code( $country_name = '' ) {
		...
		if ( !empty($country_name) ) {
            ...
            // normalize the name by converting to uppercase and removing extra spaces
            $normalized_input = strtoupper( trim($country_name) );

            // search the array for the normalized input
            foreach ( $country_codes as $country => $country_code ) {

                // normalize the country name in the array for comparison
                $normalized_country = strtoupper( trim($country) );

                if ( $normalized_input === $normalized_country ) {
                    return $country_code; // return the ISO country code if there is a match
                }
    ...
    /*
     * internal utility function to create a subscription
     */
    static function create_subscription( $subscription_data ) {
        ...
        $subscription_payment_type = $subscription_data['payment_type'];
        $subscription_payment_details = $subscription_data['payment_details'];
        ...
        //    $result['errors'][] = __td( 'You do not hold the required privileges to execute this request.' );
        //}
        ...
        if ( $valid_plan ) {
            ...
            $data_values = array(
                ...
                'payment_details' => ( $subscription_payment_details != null ? json_encode($subscription_payment_details) : $subscription_payment_details ),
                'status' => ( empty($is_free) ? ( empty($trial_days) ? 'active' : 'trial' ) : 'free' ),
                'cycle_interval' => $cycle_interval,
                'cycle_interval_count' => $cycle_interval_count,
            ...
            if ( false === $insert_result ) {
                ...
                $result['subscription_id'] = $wpdb->insert_id;
            }

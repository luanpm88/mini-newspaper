	...
	function render( $atts, $content = null ) {
		...
		$buffy = '<div class="tds-page-block ' . $block_classes . '" ' . $this->get_block_html_atts() . '>';
            ...
            $buffy .= '<div class="tds-block-inner tds-checkout-wrap' . $is_stripe_session_cls .  '">';
                ...
                $subscription_curr_name = $show_version_in_composer_confirm ? $curr_name : '';
                $subscription_curr_pos = $show_version_in_composer_confirm ? $curr_pos : '';
                $subscription_curr_th_sep = $show_version_in_composer_confirm ? $curr_th_sep : '';
                $subscription_curr_dec_sep = $show_version_in_composer_confirm ? $curr_dec_sep : '';
                $subscription_curr_dec_no = $show_version_in_composer_confirm ? $curr_dec_no : '';

                $plan_id = '';
                $plan_name = $show_version_in_composer_confirm ? 'Monthly Plan' : '';
                ...
                if ( !empty($_GET['subscription']) && !empty($_GET['key']) ) {
		            ...
		            if ( null !== $subscriptions && count($subscriptions) ) {
                        ...
                        $subscription_coupon_id = !empty($subscriptions[0]['coupon_id']) ? $subscriptions[0]['coupon_id'] : '';
                        ...
                        if ( !empty($subscription_coupon_id) ) {
                            ...
                            $formatted_price = tds_util::get_formatted_currency(
                                tds_util::get_coupon_discount( $subscription_coupon_id, $subscriptions[0]['price'] ),
                                $curr_name,
                                $curr_pos,
                                $curr_th_sep,
                                $curr_dec_sep,
                        ...
                        $subscription_trial_days = intval( $subscriptions[0]['trial_days'] );
                                ...
                                <ul class="tds-s-list">
                                    ...
                                    <li class="tds-s-list-item tds-s-subscr-period">
                                        ...
                                        <span class="tds-s-list-text">
                                            <?php echo date('Y-m-d') ?> -
                                            <?php
                                            echo !$subscription_is_free && ( !$subscription_is_unlimited || $subscription_trial_days > 0 ) ? tds_util::get_subscription_end_date( date('Y-m-d'), $subscription_cycle_interval, $subscription_cycle_interval_count, $subscription_trial_days )->format('Y-m-d') : __td('unlimited', TD_THEME_NAME )
                                            ?>
                                        </span>
                                    </li>
                ...
                if ( !empty($payment_paypal) && '1' === $payment_paypal['is_active'] ) {
                    ...
                    <script>
                        ...
                        if ( 'undefined' !== typeof window.paypal ) {
                            ...
                            let planWithTrialPeriod = '<?php echo !empty($plan_with_trial_period) ? $plan_with_trial_period : '' ?>';
                                ...
                                jQuery.ajax({
                                    ...
                                    success: function (data) {
                                        ...
                                        if ( 'undefined' !== typeof data['error'] ) {
                                            ...
                                            // @note this case applies to unlimited subscriptions only
                                            //console.log(checkoutSessionMode);
                                            if ( parseInt(planPrice) === 0 && checkoutSessionMode !== 'subscription' ) {

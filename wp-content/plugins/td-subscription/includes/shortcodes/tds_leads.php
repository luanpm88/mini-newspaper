...
class tds_leads extends td_block {
	...
	function render( $atts, $content = null ) {
        ...
        /* -- ajax -- */
        $ajax_submit = $this->get_att('ajax_submit') != '';
		...
		$buffy = '<div class="' . $block_classes . '" ' . $this->get_block_html_atts() . '>';
			...
			$buffy .= '<div class="tds-block-inner td-fix-index">';
                ...
                ?>
                    <script>
                        ...
                        jQuery().ready(function () {
                            ...
                            // Google reCaptcha.
                            tdsLeadsItem.showCaptcha = <?php echo json_encode(!empty($show_captcha)) ?>;
                            tdsLeadsItem.captchaSiteKey = '<?php echo $captcha_site_key ?>';

                            // Button texts.
                            tdsLeadsItem.subscribeBtnTxt = '<?php echo !empty($btn_text_subscribe) ? $btn_text_subscribe : 'Subscribe' ?>'
                            tdsLeadsItem.unSubscribeBtnTxt = '<?php echo !empty($btn_text_unsubscribe) ? $btn_text_unsubscribe : 'Unsubscribe' ?>'

                            // Acknowledgement checkbox.
                            tdsLeadsItem.acknowledgementCheckboxShow = <?php echo json_encode($pp_checkbox != '') ?>;
                            tdsLeadsItem.acknowledgementCheckboxMsg = '<?php echo $this->get_att('pp_msg') ?>';

                            // Redirect URLs.
                            tdsLeadsItem.subscribedRdrURL = '<?php echo $successful_submit_rdr_url ?>';
                            tdsLeadsItem.unSubscribedRdrURL = '<?php echo $unsubscribe_rdr_url ?>';

                            // AJAX submit.
                            tdsLeadsItem.ajaxSubmit = <?php echo json_encode($ajax_submit) ?>;
                            ...
                            tdsLeadsItem.messages = <?php echo json_encode( array(
                                ...
                                'success_subscribe' => __td( 'Successfully subscribed!' ),
                                'success_unsubscribe' => __td( 'Successfully unsubscribed!' ),
                                'input_placeholder' => $input_placeholder,
                                'unsubscribe_msg' => $this->get_att('unsub_msg')
	                        ) ); ?>;
                ...
                /* form */
                $buffy .= '<form class="tds-form tds-s-content tds-s-content-sm" action="" method="post" name="">';
                        ...
                        if( !$tds_leads_cookie_subscribed ) {
                            $buffy .= '<div class="tds-input-wrap">';
                                if ($show_captcha == 'show' && $captcha_site_key != '') {
                                    $buffy .= '<input type="hidden" id="g-recaptcha-response-leads-' . $this->block_uid . '" name="g-recaptcha-response" data-sitekey="' . $captcha_site_key . '">';

                ...
                if ( is_user_logged_in() || $is_composer ) {
                    ...
                    if ( $is_composer ) {
                        ...
                        if( is_page( $my_account_page_id ) ) {
                            $is_my_account_page_id = true;
                        } else if( defined( 'TD_CLOUD_LIBRARY' ) && tdb_state_template::get_template_type() == 'page' && !td_util::is_mobile_theme() && class_exists('Mobile_Detect') ) {
                            $mobile_detect = new Mobile_Detect();

                            if ( $mobile_detect->isMobile() ) {
                                $tdc_mobile_page_id = get_post_meta( $my_account_page_id, 'tdc_mobile_template_id', true );

                                if ( !empty( $tdc_mobile_page_id ) ) {
                                    global $wp_query;

                                    if( tdb_state_content::get_wp_query()->post->ID == $my_account_page_id && $wp_query->post->ID == $tdc_mobile_page_id ) {
                                        $is_my_account_page_id = true;
                                    }
                                }
                            }

		...
		switch ( tds_email_locker::instance()->get_content_lock_type() ) {
			case 'partial':
				...
				if ( preg_match( '/\[tds_partial_locker (.*?)]/', $content, $matches ) ) {
					...
					$tds_locked_content = '<div class="tds-locked-content" hidden>' . $content_after . '</div>';
			...
			case 'content':
				...
				if ( preg_match( '/' . $pattern . '/s', $content, $matches ) ) {
					...
					// apply the content locker wrapper
					$tds_locked_content = '<div class="tds-locked-content" hidden>' . $matches[5] . '</div>';
			...
			default: // full as default
				...
				$content = '<div class="tds-locked-content" hidden>' . $content . '</div>';
